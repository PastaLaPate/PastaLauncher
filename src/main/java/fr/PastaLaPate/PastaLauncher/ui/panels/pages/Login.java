/*
 * Copyright (c) 2022. BY PASTALAPATE
 */
...
public class Login extends Panel {
    ...
    public void init(PanelManager panelManager) {
        ...
        btnLogin.setOnMouseClicked(e -> this.authenticateMS(userField.getText(), passwordField.getText()));
        ...
        msLoginBtn.setOnMouseClicked(e -> this.authenticateWeb());
    ...
    public void authenticateMS(String user, String email) {
        MicrosoftAuthenticator authenticator = new MicrosoftAuthenticator();
        try {
            MicrosoftAuthResult authResult = authenticator.loginWithCredentials(user, email);
            saver.set("msAccessToken", authResult.getAccessToken());
            saver.set("msRefreshToken", authResult.getRefreshToken());
            saver.save();
            Launcher.getInstance().setAuthInfos(new AuthInfos(
                    authResult.getProfile().getName(),
                    authResult.getAccessToken(),
                    authResult.getProfile().getId()
            ));
            this.logger.info("Hello " + authResult.getProfile().getName());
